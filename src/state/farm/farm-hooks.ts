import { TEST } from 'constants/tokens'
...
export function usePools(page = 0, pageSize = 10) {
  ...
  const testPerSecondResponse = useSingleCallResult(minichefContract, 'testPerSecond', [])
  const testPerSecond = testPerSecondResponse.result?.[0]
    ? JSBI.BigInt(testPerSecondResponse.result[0].toString())
    : JSBI.BigInt(0)
  ...
  const pendingTests = useSingleContractMultipleData(minichefContract, 'pendingTest', pendingArguments, {
    blocksPerFetch: 50,
  })
      ...
      .map((poolId, idx) => {
        ...
        const poolEmissionPerSecond =
          poolInfo?.allocPoint && totalAllocation
            ? JSBI.divide(
                JSBI.multiply(testPerSecond, JSBI.BigInt(poolInfo.allocPoint.toString())),
                JSBI.BigInt(totalAllocation.toString())
              )
            : undefined

        const poolEmissionAmount = chainId
          ? CurrencyAmount.fromRawAmount(TEST[chainId], poolEmissionPerSecond || 0)
          : undefined

        const pendingResult = account
          ? (pendingTests[idx]?.result as unknown as { pending: BigNumber })
          : undefined
        ...
        const rawInfo: MinichefRawPoolInfo = {
          ...
          rewarderAddress,
          poolInfo: {
            accTestPerShare: poolInfo?.accTestPerShare,
            allocPoint: poolInfo?.allocPoint,
            lastRewardTime: poolInfo?.lastRewardTime,
          },
          pendingAmount: chainId
            ? CurrencyAmount.fromRawAmount(TEST[chainId], JSBI.BigInt(pendingResult?.pending?.toString() || 0))
            : undefined,
...
export function usePool(poolId: number) {
  ...
  const totalAllocation = totalAllocationResponse.result?.[0] as BigNumber | undefined
  const pendingTests = useSingleCallResult(minichefContract, 'pendingTest', [poolId, account ?? undefined])
  ...
  const rawInfo: MinichefRawPoolInfo | null = useMemo(() => {
    ...
    const pendingResult = account ? (pendingTests?.result as unknown as { pending: BigNumber }) : undefined
    return {
      lpTokenAddress,
      rewarderAddress,
      poolInfo: {
...
export function useRewardInfos(pid: number, rewardContractAddress?: string) {
  ...
  return {
    ...
    poolInfo: {
      accEmissionPerShare: poolInfo?.accTestPerShare,
      allocPoint: poolInfo?.allocPoint,
      lastRewardTime: poolInfo?.lastRewardTime,
    },
  }
