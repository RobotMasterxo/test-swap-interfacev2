...
export default function UnstakingModal({ isOpen, onDismiss, availableAmount }: StakingModalProps) {
  ...
  async function onStake() {
    ...
    if (testBar && parsedAmount && deadline && account) {
      try {
        const response = await testBar.leave(`0x${parsedAmount.quotient.toString(16)}`)
        addTransaction(response, {
          summary: 'Unstaked xTEST',
        })
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)} xTEST</TYPE.body>
          </AutoColumn>
        </LoadingView>
      )}
      {attempting && hash && (
        <SubmittedView onDismiss={wrappedOnDismiss} hash={hash}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>Unstaked {parsedAmount?.toSignificant(4)} xTEST</TYPE.body>
          </AutoColumn>
        </SubmittedView>
      )}
    </Modal>
