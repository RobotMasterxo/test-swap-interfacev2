...
import { useTestBar } from '../../hooks/useContract'
...
export default function StakingModal({ isOpen, onDismiss, availableAmount, currencyToAdd }: StakingModalProps) {
  ...
  const testBar = useTestBar()
  const [approval, approveCallback] = useApproveCallback(parsedAmount, testBar?.address)

  async function onStake() {
    setAttempting(true)

    if (testBar && parsedAmount && deadline && account) {
      if (approval === ApprovalState.APPROVED) {
        try {
          const response = await testBar.enter(`0x${parsedAmount.quotient.toString(16)}`)
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)} TEST</TYPE.body>
          </AutoColumn>
        </LoadingView>
      )}
      {attempting && hash && (
        <SubmittedView onDismiss={wrappedOnDismiss} hash={hash}>
          ...
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>Staked {parsedAmount?.toSignificant(4)} TEST</TYPE.body>
...
function AddXDiffButton({ addToken, success }: { addToken: () => void; success?: boolean }) {
  ...
  return <ButtonSecondary onClick={addToken}>Add xTEST to Metamask</ButtonSecondary>
}
