...
import { Qom } from '../../constants/tokens'
...
export function CurrencySearch({
  ...
  const filteredSortedTokensWithETH: Currency[] = useMemo(() => {
    ...
    if (['', 'p', 'ph', 'pho', 'phot', 'photo', 'photon', 'e', 'ev', 'evm', 'evmo', 'qom'].includes(s)) {
      return [Qom.onChain(chainId || ChainId.MAINNET), ...filteredSortedTokens]
    }
  ...
  const handleEnter = useCallback(
    (e: KeyboardEvent<HTMLInputElement>) => {
      if (e.key === 'Enter') {
        const s = debouncedQuery.toLowerCase().trim()
        if (s === 'qom' && chainId) {
          handleCurrencySelect(Qom.onChain(chainId))
