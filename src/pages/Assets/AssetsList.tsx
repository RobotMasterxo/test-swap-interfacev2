...
import { Qom, XTEST } from 'constants/tokens'
...
export const AssetsListPage = () => {
  ...
  const ethBalance = useCurrencyBalance(account ?? undefined, Qom.onChain(chainId || ChainId.MAINNET) as Currency)
  ...
  const [xtestBalance, xtestBalanceLoading] = useTokenBalancesWithLoadingIndicator(account ?? undefined, [xToken])
  const earnedTest = useEarnedTest(Object.values(xtestBalance)[0])
  const xtestValue = useUSDCValue(earnedTest)
    ...
    return (
      [ethBalance, ...Object.values(tokenBalances)]
        ...
        .filter((a) => {
          ...
          const xTest = XTEST[chainId]
          if (!xTest) {
            return true
          }
          return xTest.address !== a?.currency.wrapped.address
        })
        .filter(isTruthy)
    )
  ...
  return (
    <AssetContextProvider>
      <AssetsListContainer>
        ...
        <HeadingWithPotion
          heading="My Assets"
          description="Overview of assets, liquidity-, farm- and staking positions on Test"
        />
        <TotalAssets
          stakedBalance={parseFloat(xtestValue?.toSignificant(4) ?? '0')}
          isBalanceLoading={fetchingV2PairBalances || balancesLoading || xtestBalanceLoading || v2IsLoading}
        />
