...
import { abi as TEST_BAR_ABI } from 'abis/TestBar.json'

import { ChainId } from 'constants/chains'
import { WQOM, TEST } from 'constants/tokens'
...
export function useWETHContract(withSignerIfPossible?: boolean) {
  ...
  return useContract<Weth>(chainId ? WQOM[chainId]?.address : undefined, WETH_ABI, withSignerIfPossible)
}
...
export function useUniContract() {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId ? TEST[chainId]?.address : undefined, UNI_ABI, true)
}
...
export function useTestBar() {
  return useContract<TestBar>(TEST_BAR_ADDRESS, TEST_BAR_ABI, true)
}
